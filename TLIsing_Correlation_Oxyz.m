...
function TLIsing_Correlation_Oxyz(h,g,site)
...
Arr(:,2*midsite - 1:2*midsite) = sigmax;
Arr(:,2*midsite + 1:2*midsite + 2) = sigmay;
Arr(:,2*midsite + 3:2*midsite + 4) = sigmaz;
...
fid = fopen(['Correlation_Oxyz_site=',num2str(site),'_O_loc=',num2str(midsite),'_h=',num2str(h),'_g=',num2str(g),'.txt'],'wt');
