function Universality_0p25_of_autocorrelation
...
disp(['db[n=1]/b[n=1] = ',num2str(0.25)]);
...
bn4 = sqrt(1 + (1:dim).^2/3);
bn4 = bn4/sqrt(1 + 1/3);
H4 = diag(-bn4,1) + diag(bn4,-1);

bn5 = 1 + 2*(1:dim).^(1/4)/3 + (1:dim)/3;
bn5 = bn5/(2);
...
fidC = fopen('Universality_0p25_of_autocorrelation.txt','wt');
fidK = fopen('Universality_0p25_of_Krylov.txt','wt');
